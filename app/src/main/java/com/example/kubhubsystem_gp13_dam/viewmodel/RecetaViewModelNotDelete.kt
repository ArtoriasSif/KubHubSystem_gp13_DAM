...
class RecetasViewModel(
    ...
    val categoriasRecetas: StateFlow<List<String>> =
        recetaRepositoryNotDelete.obtenerCategorias()
            .stateIn(
                scope = viewModelScope,
                started = SharingStarted.WhileSubscribed(5000),
                initialValue = emptyList()
    ...
    fun agregarReceta(receta: Receta) {
        viewModelScope.launch {
            try {
                _isLoading.value = true
                recetaRepositoryNotDelete.agregarReceta(receta)
    ...
    fun actualizarReceta(receta: Receta) {
        viewModelScope.launch {
            try {
                _isLoading.value = true
                recetaRepositoryNotDelete.actualizarReceta(receta)
    ...
    fun eliminarReceta(idReceta: Int) {
        viewModelScope.launch {
            try {
                _isLoading.value = true
                recetaRepositoryNotDelete.eliminarReceta(idReceta)
