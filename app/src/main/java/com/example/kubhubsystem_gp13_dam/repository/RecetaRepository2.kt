...
class RecetaRepository2(
    ...
    suspend fun fetchAllActiveRecipes(forceRefresh: Boolean = false): Result<List<RecipeWithDetailsAnswerUpdateDTO>> {
        return try {
            ...
            if (!forceRefresh && isCacheValid && _cachedRecetas.value.isNotEmpty()) {
                Log.d("RecetaRepository2", "📦 Usando cache válido (${_cachedRecetas.value.size} recetas)")
            ...
            Log.d("RecetaRepository2", "🌐 Llamando API: findAllRecipeWithDetailsActive()")
            val response = recetaApiService.findAllRecipeWithDetailsActive()
            Log.d("RecetaRepository2", "✅ Respuesta API: ${response.size} recetas obtenidas")

            cacheMutex.withLock {
                _cachedRecetas.value = response
                lastFetchTimeRecetas = currentTime
                Log.d("RecetaRepository2", "💾 Cache actualizado con ${response.size} recetas")
            }
    ...
    suspend fun createRecipeWithDetails(dtoCreate: RecipeWithDetailsCreateDTO): Result<RecipeWithDetailsCreateDTO> {
        return try {
            ...
            Log.d("RecetaRepository2", "🆕 Creando receta: ${dtoCreate.nombreReceta}")
            Log.d("RecetaRepository2", "📋 DTO Create: $dtoCreate")

            val response = recetaApiService.createRecipeWithDetails(dtoCreate)
            Log.d("RecetaRepository2", "✅ Receta creada exitosamente")
    ...
    suspend fun updateRecipeWithDetails(
        ...
        return try {
            Log.d("RecetaRepository2", "---- INICIO ACTUALIZACIÓN RECETA ----")
            Log.d("RecetaRepository2", "DTO ENVIADO: $dtoUpdate")

            _isLoading.value = true
            _error.value = null

            Log.d("RecetaRepository2", "🌐 Llamando API: updateRecipeWithDetails()...")
            val recetaActualizada = recetaApiService.updateRecipeWithDetails(dtoUpdate)
            Log.d("RecetaRepository2", "✅ Respuesta API: $recetaActualizada")

            // refrescar listas si corresponde
            fetchAllActiveRecipes(forceRefresh = true)

            _isLoading.value = false
            Log.d("RecetaRepository2", "---- FIN ACTUALIZACIÓN EXITOSA ----")

            Result.success(recetaActualizada)

        } catch (e: HttpException) {
            val msg = "Error HTTP ${e.code()} al actualizar receta"
            Log.e("RecetaRepository2", msg, e)
            handleError(msg, e)
    ...
    suspend fun updateChangingStatusRecipeWith(idReceta: Int): Boolean {
        return try {
            Log.d("RecetaRepository2", "🌐 Llamando API para cambiar estado ID: $idReceta")
            val response = recetaApiService.updateChangingStatusRecipeWith(idReceta)
            val success = response.isSuccessful
            Log.d("RecetaRepository2", "✅ Respuesta: $success (code: ${response.code()})")
            success
        } catch (e: Exception) {
            Log.e("RecetaRepository2", "❌ Error al cambiar estado", e)
            false
        }
    }
    ...
    suspend fun deactivateRecipe(idReceta: Int): Result<Unit> {
        return try {
            ...
            Log.d("RecetaRepository2", "🗑️ Desactivando receta ID: $idReceta")
            val response = recetaApiService.updateStatusActiveFalseRecipe(idReceta)

            if (response.isSuccessful) {
                Log.d("RecetaRepository2", "✅ Receta desactivada exitosamente")
                ...
                cacheMutex.withLock {
                    ...
                    Log.d("RecetaRepository2", "💾 Cache actualizado (receta removida)")
                }
    ...
    suspend fun fetchProductosActivos(forceRefresh: Boolean = false): Result<List<ProductoEntityDTO>> {
        return try {
            ...
            if (!forceRefresh && isCacheValid && _cachedProductos.value.isNotEmpty()) {
                Log.d("RecetaRepository2", "📦 Usando cache de productos (${_cachedProductos.value.size} productos)")
                return Result.success(_cachedProductos.value)
            }

            Log.d("RecetaRepository2", "🌐 Llamando API: getProductsByActivo(true)")
            val response = productoApiService.getProductsByActivo(true)
            Log.d("RecetaRepository2", "✅ Respuesta API: ${response.size} productos activos obtenidos")

            productoCacheMutex.withLock {
                _cachedProductos.value = response.sortedBy { it.nombreProducto }
                lastFetchTimeProductos = currentTime
                Log.d("RecetaRepository2", "💾 Cache de productos actualizado")
            }
    ...
    suspend fun fetchUnidadesMedida(forceRefresh: Boolean = false): Result<List<String>> {
        return try {
            ...
            if (!forceRefresh && isCacheValid && _cachedUnidadesMedida.value.isNotEmpty()) {
                Log.d("RecetaRepository2", "📦 Usando cache de unidades (${_cachedUnidadesMedida.value.size} unidades)")
                return Result.success(_cachedUnidadesMedida.value)
            }

            Log.d("RecetaRepository2", "🌐 Llamando API: getUnidadesMedidaActivas()")
            val response = productoApiService.getUnidadesMedidaActivas()
            Log.d("RecetaRepository2", "✅ Respuesta API: ${response.size} unidades obtenidas")

            productoCacheMutex.withLock {
                _cachedUnidadesMedida.value = response.distinct().sorted()
                lastFetchTimeUnidades = currentTime
                Log.d("RecetaRepository2", "💾 Cache de unidades actualizado")
            }
    ...
    suspend fun updateCache(nuevaLista: List<RecipeWithDetailsAnswerUpdateDTO>) {
        cacheMutex.withLock {
            ...
            Log.d("RecetaRepository2", "💾 Cache actualizado manualmente con ${nuevaLista.size} recetas")
        }
    }
    ...
    suspend fun invalidateCache() {
        cacheMutex.withLock {
            ...
            Log.d("RecetaRepository2", "🗑️ Cache de recetas invalidado")
        }
        ...
        productoCacheMutex.withLock {
            ...
            Log.d("RecetaRepository2", "🗑️ Cache de productos invalidado")
        }
    }
...
// ========= UTILIDADES PRIVADAS =========
    ...
    private fun <T> handleError(message: String, e: Exception): Result<T> {
        ...
        Log.e("RecetaRepository2", message, e)
    ...
    class RecetasViewModelFactory2(
        ...
        override fun <T : ViewModel> create(modelClass: Class<T>): T {
            if (modelClass.isAssignableFrom(RecetasViewModel2::class.java)) {
                return RecetasViewModel2(recetaRepository2) as T
            }
